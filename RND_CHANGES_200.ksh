...
do
	...
	echo "$MKT Rnd Counts"
	scp tlpvebc1:/tlgvar$MKT/$MKT/log/RELEASE_SP*$CURDAY*.log .
...
do
	MKTUP=`echo $MKT | tr  "[:lower:]" "[:upper:]"`
	echo "$MKT Rnd Counts"
	scp tlpvebc2:/tlgvar$MKT/$MKT/log/RELEASE_SP*$CURDAY*.log .
	for x in `ls -1 RE*SP*$CURDAY*$MKTUP*.log`
	do
		grep -i 'Market: ' $x > Market.txt
		grep -E "Num of rows for table|, in refw" $x > "$MKTUP"_rel.txt
...
# do
	# MKTUP=`echo $MKT | tr  "[:lower:]" "[:upper:]"`
	# echo "$MKT Rnd Counts"
	# scp tlpvebc2:/tlgvaraus/$MKT/log/RELEASE_SP*$CURDAY*.log .
	# grep -i 'Market: ' RE*SP*$CURDAY_$MKTUP*.log > Market.txt
	# grep -E "Num of rows for table|, in refw" RE*SP*$CURDAY_$MKTUP*.log > "$MKTUP"_rel.txt
	# awk '{ $8 = ""; $9 = ""; $10 = ""; print}' "$MKTUP"_rel.txt > "$MKTUP"_rel_test.txt
	# cat "$MKTUP"_rel_test.txt |  awk '$16 > 0 ' >> "$MKTUP"_release_changes.txt
	# cat Market.txt "$MKTUP"_release_changes.txt > "$MKTUP"_release.txt
...
do
	MKTUP=`echo $MKT | tr  "[:lower:]" "[:upper:]"`
	echo "$MKT Rnd Counts"
	scp tlpvebc3:/tlgvar$MKT/$MKT/log/RELEASE_SP*$CURDAY*.log .
	for x in `ls -1 RE*SP*$CURDAY*$MKTUP*.log`3
	do
		grep -i 'Market: ' $x > Market.txt
		grep -E "Num of rows for table|, in refw" $x > "$MKTUP"_rel.txt
		awk '{ $8 = ""; $9 = ""; $10 = ""; print}' "$MKTUP"_rel.txt > "$MKTUP"_rel_test.txt
...
do
	MKTUP=`echo $MKT | tr  "[:lower:]" "[:upper:]"`
	echo "$MKT Rnd Counts"
	scp tlpvebe1:/tlgvar$MKT/$MKT/log/RELEASE_SP*$CURDAY*.log .
	for x in `ls -1 RE*SP*$CURDAY*$MKTUP*.log`
	do
		grep -i 'Market: ' $x > Market.txt
		grep -E "Num of rows for table|, in refw" $x > "$MKTUP"_rel.txt
...
do
	MKTUP=`echo $MKT | tr  "[:lower:]" "[:upper:]"`
	echo "$MKT Rnd Counts"
	scp Ulpvebc5.madc.att.com:/tlgvar$MKT/$MKT/log/RELEASE_SP*$CURDAY*.log .
	for x in `ls -1 RE*SP*$CURDAY*$MKTUP*.log`
	do
		grep -i 'Market: ' $x > Market.txt
		grep -E "Num of rows for table|, in refw" $x > "$MKTUP"_rel.txt
...
do
	MKTUP=`echo $MKT | tr  "[:lower:]" "[:upper:]"`
	echo "$MKT Rnd Counts"
	scp tlpvebp1.dadc.sbc.com:/tlgvar$MKT/$MKT/log/RELEASE_SP*$CURDAY*.log .
	for x in `ls -1 RE*SP*$CURDAY*$MKTUP*.log`
	do
		grep -i 'Market: ' $x > Market.txt
		grep -E "Num of rows for table|, in refw" $x > "$MKTUP"_rel.txt
...
do
	MKTUP=`echo $MKT | tr  "[:lower:]" "[:upper:]"`
	echo "$MKT Rnd Counts"
	scp olpvebe6.bodc.att.com:/tlgvar$MKT/$MKT/log/RELEASE_SP*$CURDAY*.log .
	for x in `ls -1 RE*SP*$CURDAY*$MKTUP*.log`
	do
		grep -i 'Market: ' $x > Market.txt
		grep -E "Num of rows for table|, in refw" $x > "$MKTUP"_rel.txt
...
do
	MKTUP=`echo $MKT | tr  "[:lower:]" "[:upper:]"`
	echo "$MKT Rnd Counts"
	scp blpvebc8.bhdc.att.com:/tlgvar$MKT/$MKT/log/RELEASE_SP*$CURDAY*.log .
	for x in `ls -1 RE*SP*$CURDAY*$MKTUP*.log`
	do
		grep -i 'Market: ' $x > Market.txt
		grep -E "Num of rows for table|, in refw" $x > "$MKTUP"_rel.txt
...
do
	MKTUP=`echo $MKT | tr  "[:lower:]" "[:upper:]"`
	echo "$MKT Rnd Counts"
	scp tlpvebm1.dadc.sbc.com:/tlgvar$MKT/$MKT/log/RELEASE_SP*$CURDAY*.log .
	for x in `ls -1 RE*SP*$CURDAY*$MKTUP*.log`
	do
		grep -i 'Market: ' $x > Market.txt
		grep -E "Num of rows for table|, in refw" $x > "$MKTUP"_rel.txt
...
do
	MKTUP=`echo $MKT | tr  "[:lower:]" "[:upper:]"`
	echo "$MKT Rnd Counts"
	scp blpvebe8.bhdc.att.com:/tlgvar$MKT/$MKT/log/RELEASE_SP*$CURDAY*.log .
	for x in `ls -1 RE*SP*$CURDAY*$MKTUP*.log`
	do
		grep -i 'Market: ' $x > Market.txt
		grep -E "Num of rows for table|, in refw" $x > "$MKTUP"_rel.txt
...
do
	MKTUP=`echo $MKT | tr  "[:lower:]" "[:upper:]"`
	echo "$MKT Rnd Counts"
	scp olpvebc6.bodc.att.com:/tlgvar$MKT/$MKT/log/RELEASE_SP*$CURDAY*.log .
	for x in `ls -1 RE*SP*$CURDAY*$MKTUP*.log`
	do
		grep -i 'Market: ' $x > Market.txt
		grep -E "Num of rows for table|, in refw" $x > "$MKTUP"_rel.txt
...
cat INREGION_release.txt OUTREGION_release.txt AWE_NBI_release.txt NESE_release.txt > $CURDAY_Release.txt
